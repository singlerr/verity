...
pub fn render_status_bar(f: &mut Frame, area: Rect, app: &App) {
    ...
    let mut left_spans: Vec<Span> = vec![
        ...
        Span::raw(" "),
        Span::styled(state_str.to_string(), Style::default().fg(colors.ink).add_modifier(Modifier::BOLD)),
        sep.clone(),
        Span::styled(format!("tokens \u{2014} \u{b7} {}", elapsed), Style::default().fg(colors.dim)),
        sep.clone(),
