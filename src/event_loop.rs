...
fn handle_model_select(app: &mut App, code: KeyCode) {
    match code {
        ...
        KeyCode::Enter => {
            if let Some(model) = app.model_list.get(app.selected_model_idx).cloned() { set_model(app, model); }
...
fn autocomplete_next(app: &mut App, forward: bool) {
    ...
    app.query = crate::ui::autocomplete::COMMANDS[matches[app.autocomplete_idx]].0.to_string();
}

fn handle_global_key(code: KeyCode, mods: crossterm::event::KeyModifiers, app: &mut App, tx: &std::sync::mpsc::Sender<AgentEvent>) -> bool {
    ...
    match code {
        ...
        KeyCode::Char('/') if !in_cmd => { app.focus = Focus::Command; app.query.push('/'); true }
        KeyCode::Char('m') if !in_cmd => { open_model_select(app, tx); true }
        KeyCode::Esc if in_cmd => { app.focus = Focus::Left; true }
        KeyCode::Enter if in_cmd && !app.query.is_empty() => { handle_command(app, tx); true }
        KeyCode::Tab | KeyCode::Down if in_cmd => { autocomplete_next(app, true); true }
        KeyCode::Up if in_cmd => { autocomplete_next(app, false); true }
        KeyCode::Char(c) if in_cmd => { app.query.push(c); app.autocomplete_idx = 0; true }
        KeyCode::Backspace if in_cmd => { app.query.pop(); app.autocomplete_idx = 0; true }
        _ => false,
    }
}
...
fn handle_focus_key(code: KeyCode, app: &mut App) {
    ...
    match code {
        KeyCode::Tab if f != Focus::Command =>
            app.focus = if f == Focus::Left { Focus::Right } else { Focus::Left },
        ...
        KeyCode::Char('k') if f == Focus::Left => {
            ...
            else if let Some(s) = app.selected_source { app.selected_source = Some(s.saturating_sub(1)); }
        }
        KeyCode::Char('j') | KeyCode::Down if f == Focus::Right => app.answer_scroll = app.answer_scroll.saturating_add(1),
        KeyCode::Char('k') | KeyCode::Up if f == Focus::Right => app.answer_scroll = app.answer_scroll.saturating_sub(1),
        _ => {}
    }
}
...
fn handle_command(app: &mut App, agent_tx: &std::sync::mpsc::Sender<AgentEvent>) {
    ...
    match cmd {
        "/model" => {
            app.query.clear(); app.focus = Focus::Left;
            if args.is_empty() { open_model_select(app, agent_tx); }
            else { set_model(app, args.to_string()); }
        }
        "/mode" => {
            app.mode = match args { "research" => Mode::Research, "code" => Mode::Code, _ => app.mode.clone() };
            app.query.clear(); app.focus = Focus::Left;
        }
        _ => {
            ...
            tokio::spawn(async move {
                AgentOrchestrator::new(planner, tools, pr, model).run(&query, tx, cancel_token).await;
            });
        }
    }
}
