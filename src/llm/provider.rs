...
pub mod mock {
    ...
    impl MockLlmProvider {
        pub fn new(responses: Vec<ToolResponse>) -> Self {
            Self {
                ...
                stream_responses: Arc::new(Mutex::new(vec![vec![Chunk { content: "mock synthesis".into() }]])),
    ...
    impl LlmProvider for MockLlmProvider {
        async fn stream_completion(&self, _messages: &[Message], _model: &str) -> Result<Vec<Chunk>, ProviderError> {
            let mut responses = self.stream_responses.lock().unwrap();
            if responses.is_empty() { Ok(vec![Chunk { content: "mock".into() }]) }
            else { Ok(responses.remove(0)) }
        }
        async fn complete_with_tools(&self, _messages: &[Message], _tools: &[ToolDefinition], _model: &str) -> Result<ToolResponse, ProviderError> {
            let mut count = self.call_count.lock().unwrap();
            *count += 1;
            let mut responses = self.responses.lock().unwrap();
            if responses.is_empty() { Err("No more responses".into()) }
            else { Ok(responses.remove(0)) }
        }
        ...
        async fn list_models(&self) -> Result<Vec<String>, ProviderError> { Ok(vec!["mock-model".into()]) }
        fn supports_tool_calling(&self) -> bool { self.tool_calling_support }
    }
}
...
mod tests {
  ...
  fn non_openai_provider_rejected() {
    ...
    assert!(!non_openai.supports_tool_calling(), "Non-OpenAI provider should not support tool calling");

    let openai_compatible = MockLlmProvider::new(vec![]);
    assert!(openai_compatible.supports_tool_calling(), "OpenAI-compatible provider should support tool calling");
  }
  ...
  fn mock_provider_stream_completion() {
    ...
    rt.block_on(async {
      ...
      let stream2 = vec![
        Chunk { content: "Second ".into() },
        Chunk { content: "response".into() },
      ];
